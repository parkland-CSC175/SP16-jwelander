...
var addContent = function(articleContent, aDivision){
        if (articleContent.heading !== ""){
            var aHeading = document.createElement("h2");
            aDivision.appendChild(aHeading);
            aHeading.innerHTML = articleContent.heading;
        }
        var aParagraph = document.createElement("p");
        aDivision.appendChild(aParagraph);
        aParagraph.innerHTML = articleContent.paragraph;
}

var addArticle = function(anArticle) {
    var anArticleDiv = document.createElement("div");
    anArticleDiv.className = "article-wrapper";
    divContent.appendChild(anArticleDiv);
    var aTitle = document.createElement("h1");
    anArticleDiv.appendChild(aTitle);
    aTitle.innerHTML = anArticle.title;
    var anAuthor = document.createElement("h4");
    anArticleDiv.appendChild(anAuthor);
    anAuthor.innerHTML = anArticle.author;
    var aPublisher = document.createElement("h4");
    anArticleDiv.appendChild(aPublisher);
    aPublisher.innerHTML = anArticle.publishedOn;
    var contentDiv = document.createElement("div");
    contentDiv.className = "content-wrapper";
    anArticleDiv.appendChild(contentDiv);
    for (var j = 0; j < anArticle.content.length; j++){
        addContent(anArticle.content[j], contentDiv);
    } 
}

var divContent = document.getElementById("blog-container");
